...
typedef void (^EPPZReachCompletionBlock)();
...
@property (nonatomic, strong) EPPZReachCompletionBlock completion;
...
//{ [self reachHost:hostNameOrAddress completion:completition]; }
//
//+(void)reachHost:(NSString*) hostNameOrAddress withPort:(u_short)aPort completion:(EPPZReachabilityCompletionBlock) completion
//{
//    EPPZRLog(@"EPPZReachability ----------");
//    EPPZRLog(@"EPPZReachability reachHost: '%@'", hostNameOrAddress);
//    
//    EPPZReachability *instance = [[self alloc] initWithHost:hostNameOrAddress andPort:aPort];
//    instance.completion = completion;
//    
//    [self addReachabilityToIndex:instance]; // Retain
//    [instance reachWithCompletion:^(EPPZReachability* reachability)
//    {
//        // Do not tear down
//        if (reachability.completion) reachability.completion(reachability);
//        [reachability tearDown]; // Release
//    }];
//}

+(void)reachObservedHost:(NSString*) hostNameOrAddress andPort:(u_short)aPort completion:(EPPZReachabilityCompletionBlock) completion
{
    EPPZRLog(@"EPPZReachability ----------");
    EPPZRLog(@"EPPZReachability reachObservedHost: '%@'", hostNameOrAddress);
    
    EPPZReachability *instance = [[self alloc] initWithHost:hostNameOrAddress andPort:aPort]; // Probably there is a reachability allocated out there
    instance.completion = completion;
...
+(void)listenHost:(NSString*) hostNameOrAddress withPort:(u_short)aPort delegate:(id<EPPZReachabilityDelegate>) delegate
{
    EPPZRLog(@"EPPZReachability ----------");
...
-(id)initWithHost:(NSString*) hostNameOrAddress andPort:(u_short)aPort
{
    if (self = [super init])
    {
        ...
        if ([hostNameOrAddress isIPaddress]) // Pretty neat IP address regular expression check.
        {
            ...
            hostAddress.sin_port = htons(aPort);
...
-(void)reachWithCompletion:(EPPZReachabilityCompletionBlock) completion
{
    ...
    dispatch_async(dispatch_queue_create("com.eppz.reachability.reach", NULL), ^
    {
        ...
        if (SCNetworkReachabilityGetFlags(self.reachabilityRef, &flags))
        {
            ...
            dispatch_async(dispatch_get_main_queue(), ^ // Dispatch completion callback on the main thread
            {
...
+(NSMutableArray*)reachabilityIndex
{
...
+(void)addReachabilityToIndex:(EPPZReachability*) reachability
{
    ...
    EPPZRLog(@"EPPZReachability reachabilityCount: <%lu>", (unsigned long)[[self reachabilityIndex] count]);
}
...
+(void)stopListeningHost:(NSString*) hostNameOrAddress withPort:(u_short)aPort delegate:(id<EPPZReachabilityDelegate>) delegate
{
